...
const Post = ({ id, contenido, fecha, autoria, onChange }) => {
  ...
  useEffect(
    () => () => {
      console.log("Se ha eliminado un post");
    },
    []
  );
...
const useFlag = (valorDefecto = false) => {
  ...
  return [valor, () => setValor(true), unset];
};
...
function App() {
  ...
  useEffect(() => {
    console.log("useEffect getBlog");
    getBlog().then(setBlog);
    return () => {
      console.log("useEffect getBlog cleanup");
      unset();
    };
  }, [obsoleto, unset]);
