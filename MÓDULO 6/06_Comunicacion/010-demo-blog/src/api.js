...
export const getBlog = () => api("/posts").then(ps => 
  ps.sort((a, b) => Date.parse(b.fecha) - Date.parse(a.fecha))
)
export const getAuthor = autorId => api(`/cuentas/${autorId}`)
export const editPost = (id, contenido) => api(`/posts/${id}`, "PATCH", { contenido })
export const saveNewPost = contenido => api("/posts", "POST", {
