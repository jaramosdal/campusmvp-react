...
export const OpcionA = conEstilo(a, ui.opcion)
...
export const BarraMenu = ({ izq, dcha, secundaria, children, ...props }) => (
  <div className={`sticky left-0 w-[100vw] px-6 py-2 flex flex-row gap-3 flex-wrap 
    ...
    <div className="grow min-w-max">{ izq }</div>
    <div className="grow flex flex-row flex-wrap gap-3 justify-evenly sm:justify-end items-baseline">{ dcha }</div>
  </div>
)
...
export const CampoBoton = ({ input, boton, className }) => {
  ...
  return <div className={`flex flex-row gap-0 w-full ${className ?? ""}`}>
    <input className={`grow focus:rounded-none focus:rounded-l-lg 
          ...
          focus:bg-gray-50 dark:focus:bg-gray-700 grow min-w-[5rem] peer ${inputClass ?? ""}`} { ...inputProps } />
    <Boton className={`rounded-none rounded-r-lg hidden peer-focus:inline ${botonClass ?? ""}`} { ...botonProps } />
  </div>
}